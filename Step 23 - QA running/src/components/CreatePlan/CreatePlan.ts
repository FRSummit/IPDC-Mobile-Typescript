...
export default class DashboardTypeScript extends Vue {
    ...
    async copyPlan() {    
        ...
        const year = this.selectedOrgReportingPeriod[0].split('-')[0];
        const reportingmonth = this.selectedOrgReportingPeriod[0].split(' ')[1];
    ...
    onUnitPlanCopied = (planid: number) => {
        ons.notification.toast('Plan copied',{ timeout: 1000, animation: 'fall' }); 
        this.navigate(planid);
    }
    onUnitPlanCopyFailed = (e: {$values: string[]}) => { 
        console.log(e.$values.join(", "));
        ons.notification.toast(e.$values.join(", "),{ timeout: 1000, animation: 'fall' });
        this.progressBar = false;      
    } 

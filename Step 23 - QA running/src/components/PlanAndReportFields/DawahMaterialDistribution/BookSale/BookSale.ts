...
export default class BookSale extends Vue {
    ...
    async onSubmit() {
      ...
      if(this.planOrReportTab === 'PLAN') {
        ...
        this.unitPlanModifiedData.bookSaleMaterialPlanData = this.bookSaleMaterialPlanData;
        if (this.isPlanDirty()) {
          try {
        ...
        this.unitReportModifiedData.bookSaleMaterialData = this.bookSaleMaterialReportlData;
    ...
    nextBtn() {
      let tab = document.querySelectorAll('.my_tab')[1] as HTMLElement
      this.$router.replace('/plan-and-report-edit/dawah-material-distribution/book-distribution')
      tab.click()
    }
    mounted() {
